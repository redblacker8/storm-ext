...
class PelisplusOrgProvider : MainAPI() {
    override var mainUrl = "https://pelis-plus.icu"
        ...
        urls.apmap { (name, url) ->
            ...
            val home =
                doc.select("div.app div.main_cont div.main_lista div.main_lista_box a.card").map {
                    val title = it.selectFirst("div.card_title p")?.text()
                    val link = it.attr("href")
                    val img =
                        it.selectFirst("div.card_imagen figure span.card_img img")?.attr("img")
                    val year = it.selectFirst("div.card_imagen figure span.card_anio")?.attr("data")
                        ?.toIntOrNull()
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("div.app div.main_cont div.main_lista div.main_lista_box a.card").map {
            val title = it.selectFirst("div.card_title p")?.text()
            val link = it.attr("href")
            val img = it.selectFirst("div.card_imagen figure span.card_img img")?.attr("img")
            val year =
                it.selectFirst("div.card_imagen figure span.card_anio")?.attr("data")?.toIntOrNull()
            TvSeriesSearchResponse(
                title!!,
                link!!,
                this.name,
